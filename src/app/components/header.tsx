...
function LocationLink({ location, locationLink }: LocationLinkProps) {
  return (
    <p className="max-w-md items-center text-pretty font-mono text-xs text-foreground/80">
      <a
        className="inline-flex items-center gap-x-1.5 align-baseline leading-none hover:text-primary"
        href={locationLink}
        target="_blank"
...
function SocialButton({ href, iconType, label }: SocialButtonProps) {
  ...
  return (
    <Button
      className="size-8 border-primary/20 bg-white/80 text-primary shadow-sm hover:border-primary/45 hover:bg-white"
      variant="outline"
      size="icon"
      asChild={true}
    >
  ...
  return (
    <header className="flex items-start justify-between gap-6">
      <div className="flex-1 space-y-1.5">
        ...
        <p className="max-w-md text-pretty font-medium text-sm text-primary print:text-[12px]">
          {RESUME_DATA.about}
        </p>
      ...
      <Avatar
        className="size-24 ring-2 ring-primary/15 md:size-28"
